...
func TestResourceKey(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if string(got) != tt.want {
			t.Errorf("NewResourceKey(%q, %q, %d) = %q, want %q", tt.kind, tt.repo, tt.prNumber, got, tt.want)
		}
	}
}
...
func TestPruneNilLiveness(t *testing.T) {
	tr := New(nil)
	tr.Register(NewResourceKey("repo", "foo", 0), "%1", PaneShell)
...
func TestAllPanes(t *testing.T) {
	tr := New(nil)
	tr.Register(NewResourceKey("repo", "a", 0), "%1", PaneShell)
	tr.Register(NewResourceKey("repo", "b", 0), "%2", PaneAgent)
	tr.Register(NewResourceKey("repo", "a", 0), "%3", PaneAgent)
...
func TestCountForResourceEmpty(t *testing.T) {
	tr := New(nil)
	shells, agents := tr.CountForResource(NewResourceKey("repo", "nonexistent", 0))
