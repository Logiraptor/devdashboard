...
- `SPC a a` triggers agent run for current project
...
- Shown when user triggers `SPC a a`; dismissed with Esc
...
1. **Pane layout**: devdeploy runs in one pane. `SPC a a` creates new pane via `tmux split-window -c <workDir>` with shell in project directory.
...
2. `SPC a a` → ProgressWindow (or tmux pane) appears
...
3. Help: After `SPC` → hints show `p`, `q`, `a`; after `SPC p` → `c`, `d`, `a`, `r`
